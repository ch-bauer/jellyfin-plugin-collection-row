...
native Jellyfin movie cards** â€” clickable, scrollable with arrows, with the full native hover
...
## Configuration (Dashboard â†’ Plugins â†’ Collection Section)
...
- [File Transformation plugin](https://github.com/IAmParadox27/jellyfin-plugin-file-transformation)
  â‰¥ 2.5.x â€” install it from the repository `https://www.iamparadox.dev/jellyfin/plugins/manifest.json`
  (Dashboard â†’ Plugins â†’ Repositories), then restart Jellyfin.

## Install from plugin repository (recommended)

1. Dashboard â†’ Plugins â†’ Repositories â†’ add
...
2. Commit, tag `v<version>` (e.g. `v1.2.0.0`), push the tag â€” the GitHub Actions workflow builds
...
- A startup task registers a transformation for `index.html` with the File Transformation plugin
  ...
  JS/CSS into the page head â€” jellyfin-web itself is never modified on disk.
- The injected script watches for the item detail view, asks the plugin's API
  (`GET /CollectionSection/Collections?itemId=â€¦`) which collections contain the item (a reverse
  ...
  them using Jellyfin's own card markup and `emby-scroller` / `emby-itemscontainer` elements â€”
